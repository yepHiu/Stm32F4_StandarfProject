...
{
	//	float temp = (DataofPC[sequence] << 24) | (DataofPC[sequence + 1] << 16) | (DataofPC[sequence + 2] << 8) | (DataofPC[sequence + 3] << 0);
	float_union HexToFloat;
	HexToFloat.sv[0] = (((unsigned char)DataofPC[sequence + 1]) << 8 | DataofPC[sequence]);
	HexToFloat.sv[1] = (((unsigned char)DataofPC[sequence + 3]) << 8 | DataofPC[sequence + 2]);
