...
void UART1_task(void *p_arg)
{
    ...
    while(1)
    {
		...
		DMA_USART2_Send(drone_msg.buf,drone_msg.len);
...
while(1)
	{
		err=OS_ERR_NONE;
		OSSemPend (&uart2_proc,100,OS_OPT_PEND_BLOCKING,0,&err);
		if(err == OS_ERR_TIMEOUT)
		{
			LED2_ON;
		}
		...
		//DMA_USART2_Send(u_cam_data.buf,u_cam_data.len);
...
void UART3_task(void *p_arg)//hcam
{
    ...
    while(1)
	{
		OSSemPend (&uart3_proc,100,OS_OPT_PEND_BLOCKING,0,&err);
		if(err == OS_ERR_TIMEOUT)
		{
			LED3_ON;
		}
		...
		//DMA_USART2_Send(h_cam_data.buf,h_cam_data.len);
...
void UART4_task(void *p_arg)
{
    ...
    while(1)
	{
		OSSemPend (&uart4_proc,100,OS_OPT_PEND_BLOCKING,0,&err);
		if(err == OS_ERR_TIMEOUT)
		{
			LED4_ON;
		}
		...
		//DMA_USART2_Send(Flow_data.buf,Flow_data.len);
...
void UART5_task(void *p_arg)
{
    ...
    while (1)
    {
        ...
        else if (err == OS_ERR_NONE)
        {
			...
			//DMA_USART2_Send(Tof_data.buf,Tof_data.len);
